...
domapi.rpc._dispatchAsGet = function(p,timeout){ // return: 1=success -1=error -2=nothing to do
  try{
    ...
    url    += ((p.url.indexOf("?")!=-1)?"&":"?") +
              ...
              "&ds=" + new Date().getTime() +
              "&method=GET" +
              "&" + d.saveToURL("&","=",true); // $todo: (cst) was saveToString
...
domapi.rpc._dispatchAsPost = function(p,timeout){ // return: 1=success -1=error -2=nothing to do
  ...
  try{
    ...
    if(this.doDebug){alert("Dispatching: "+url);window.open(url,"debugwin","status=yes,toolbar=yes,",null);} // $todo: (cst) added name "debugwin" for debugging rpc in a new window
    var temp = '<html><body>'+
	...
	if(this.doDebug){ doc.forms['rpcForm'].target="debugwin"; doc.forms['rpcForm'].submit(); } // $todo: (cst) send also the parameter by post to the debug window
...
domapi.rpc._createDispatcher = function(){
  if(this.dispatcher)
  		if(domapi.isIE&&!domapi.isIE5&&typeof(this.dispatcher.contentWindow.document.namespaces)!="object") // $todo: (cst) added if statement for IE6 and dispatcher corrupt
			document.body.removeChild(this.dispatcher); // $todo: (cst) remove iframe / dispatcher and create a new one
