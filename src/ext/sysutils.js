...
//------------------------------------------------------------------------------
sysutils.bintodec = function(s){
  return parseInt(s,2);
};
//------------------------------------------------------------------------------
sysutils.dectobin = function(i){
...
sysutils.makeFourDigitYear = function(s){ // takes yy and tries to output yyyy
  ...
  if(s<50) return "20"+sysutils.padZeros(s,2);
  else return "19"+s;
};
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
sysutils.addDaysToDate = function(d,n){
  return d.setDate(d.getDate() + parseInt(n));
};
/*sysutils.addDaysToDate = function(d,n){
  d=new Date(d);
  ...
  if (tempday<1){
    month--;
    if (month<0) month=11;
    tempday=sysutils.getMonthLength(month,year)-tempday;
  }
  if (tempday>sysutils.getMonthLength(month,year)){
    tempday=sysutils.getMonthLength(month,year)-tempday;
    month++;
    if (month>11) month=0;
  }
  return sysutils.createDate(month,tempday,year);
};*/
//------------------------------------------------------------------------------
...
sysutils.isDate = function(d,f){ // returns false if not a date or returns the date as a Date object
  ...
  for(var a=0;a<f.length;a++){
    ...
    if(f[a].charAt(0)=="Y")yr = sysutils.padZeros(d[a],f[a].length);
  }
...
// utility functions
//------------------------------------------------------------------------------
sysutils.removeTags = function(s, intWorkFlow){ // J�hann Haukur Gunnarsson joi@innn.is
