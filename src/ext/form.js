...
// private members
//------------------------------------------------------------------------------
domapi.form._isSpecialKey = function(k){
  switch(k){
    case  0 : // unknown (null)
    case  8 : // backspace
    case  9 : // tab
...
domapi.form._autocompleteKey = function(E){
  ...
  if(domapi.form._isSpecialKey(k) || ctrlKey)return true; // take no action, let the keystroke surface
...
domapi.form._validChar = function(vC, M, i){
  ...
  var mC = M.charAt(i);
  switch(mC){
    case "_" : return true; break;                // match
...
domapi.form._maskededitKeyup = function(E){
  ...
  var ctrlKey = domapi.isIE?(event.ctrlKey):(E.ctrlKey || E.altKey);
  if(domapi.form._isSpecialKey(k) || ctrlKey){
    if(e.isComplete()){
