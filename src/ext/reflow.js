...
domapi.reflow.render = function(p){
  ...
  try{
    try{
      ...
      if(domapi.trace)dump( [p?p.toString():'null','reflow elm:'+this.toString()+'.reflow.render() Start Reflow Task: '+this._passSession]);
      this._render(p);
    }catch(E){if(domapi.trace)dump(' render() Error :'+ E.message);};
  }finally{
...
domapi.reflow._render = function(p){
  ...
  try{
   e   = elms[i];
    ...
    if(domapi.trace)dump('reflow._render() Error :'+ e.message);
...
if(domapi.trace)dump(this.toString()+' getY('+[ppos,this.B,this.H,this.M]+')');
