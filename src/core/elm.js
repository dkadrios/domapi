...
domapi.createIFrame = function(arg){
  ...
  e.getDocument = function(){
    var doc;
    if(domapi.isIE && domapi.major==5)doc = this.document;
    else doc = domapi.isIE?this.contentWindow.document:this.contentDocument;
    return doc;
  };
  e.Hide        = function(){
...
domapi._freeElm = function(e){    // Modified by Krister ...
  if(e && !e.freed){
    try{
      ...
      e = null;
    }catch(E){throw new Error('error in _freeElm - ' + E.message);};
  }
};
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
domapi.elmProto.regHook = domapi.regHook; /* function(nm,func){
  var h = this.hooks[nm];
  if(h)h[h.length] = func;
  else this.hooks[nm] = [func];
};*/
//------------------------------------------------------------------------------
domapi.elmProto.unRegHook = domapi.unRegHook; /* function(nm,func){ // M. Proctor
  // darin - added: returns 1 for success, 0 for error
  var h = this.hooks[nm]; if(!h)return 0;
  var i = h.indexOf(func);
  if(i==-1)return 0;
...
//------------------------------------------------------------------------------
domapi.elmProto.setSizeBy   = function(w,h){
  this.setSize(this.getW()+parseInt(w),this.getH()+parseInt(h));
  this._dispHook("setSizeBy",arguments);
};
//------------------------------------------------------------------------------
domapi.elmProto.moveTo      = function(x,y){
  this.setPosition();
...
//------------------------------------------------------------------------------
domapi.elmProto.moveBy      = function(x,y){
  this.moveTo( this.getX()+parseInt(x),this.getY()+parseInt(y));
  if(domapi.useElmHooks)this._dispHook("moveBy",arguments);
};
//------------------------------------------------------------------------------
domapi.elmProto.moveToElm   = function(e){
  this.moveTo(e.getX(),e.getY());
  if(domapi.useElmHooks)this._dispHook("moveToElm",arguments);
};
//------------------------------------------------------------------------------
domapi.elmProto.sizeToElm   = function(e){
  this.setSize(e.getW(),e.getH());
