...
domapi.getContent = function(url){
  ...
  try{
    ...
    if(domapi.isIE   )req = new ActiveXObject(domapi.msXmlValidProgId);
    if(domapi.isGecko)req = new XMLHttpRequest();
...
domapi.postContent = function(url,obj){
  try{
    ...
    var _url = url + ((url.indexOf("?")!=-1)?"&":"?") + "ds=" + new Date().getTime();
    if(domapi.isIE   )req = new ActiveXObject("Microsoft.XMLHTTP");
    if(domapi.isGecko)req = new XMLHttpRequest();
