...
domapi.comps.splitpane.constructor = function(arg){
  ...
  var src2  = arg["src2" ];
  //-----------------------------------------------
  if(src1)type1 = "IFRAME";  if(id1)ref1 = domapi.getElm(id1);
  if(src2)type2 = "IFRAME";  if(id2)ref2 = domapi.getElm(id2);
  ...
  for(i=0;i<2;i++){
    ...
    if(s)setTimeout('document.getElementById("' + p.id + '").src = "' + s + '"', 250);
    if(p.tagName == "IFRAME"){
      p.border                = 0;
      p.frameBorder           = 0;
...
domapi.comps.splitpane._layout = function(w,h){
  ...
  //if(p1._layout)p1.layout(w,h);
  //if(p2._layout)p2.layout(w,h);
};
//------------------------------------------------------------------------------
...
domapi._private.splitpane.dosplittermouseout = function(E){
  var e = domapi.findTarget(E,"SPLITTER");if(!e)return;
  var p = e.parentNode;
