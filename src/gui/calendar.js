...
domapi.comps.calendar.calendarDraw = function(){
  ...
  for(var i=0;i<7;i++){
    j = i + firstWeekDay;
    if(j > 6)j -= 7;
    this.cols[j].childNodes[0].innerHTML = weekArray[j];
  }
...
domapi.comps.calendar.setDay   = function(d,preserve,range){this.setValue(new Date(this.y, this.m, d), "D", preserve, range)};
...
domapi._private.calendar.domouseover = function(E){
  var e = domapi.findTarget(E,"DIV");if(!e || !sysutils.isInteger(e.innerHTML))return;
