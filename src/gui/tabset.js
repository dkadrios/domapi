...
domapi.comps.tabset.constructor = function(arg){
  ...
  e.onchange         = function(i,value){}; // $todo: (cst) added value
  e.tabs           = [];
  e.index          = -1;
  e.rows           = [];
  e.style.overflow = "hidden";
  e.selectTab	   = domapi.rInt(arg["selectTab"],"0"); // $todo: (cst) added
...
domapi.comps.tabset._draw = function(){
  this.tabsetDraw();
  this._resizeTabs(); // $todo: (cst) new, call the resize function
};
//------------------------------------------------------------------------------
...
domapi.comps.tabset.setIndex = function(i){
  ...
  if(this.onchange)this.onchange(i,t[i].value); // todo: (cst) added t[i].value to onchange function
  this._layoutTabs();
  this.managePanes();
  return true;
};
...
domapi.comps.tabset.addTab = function(arg){
  ...
  //var value;   // $todo: (cst) no longer used
  //if(arg["value"])value = arg["value"];   // $todo: (cst) no longer used
  //if(arg["text" ])value = arg["text" ];   // $todo: (cst) no longer used
  var pane = arg["pane"];
  var e = document.createElement("LI");
  e.innerHTML    = '<span>' + domapi.rVal(arg["text"]) + '</span>';  // $todo: (cst) was value
  e.value        = domapi.rVal(arg["value"],arg["text"]); // $todo: (cst) was value, setting of value after setting of innerHTML!! (because IE crashes)
  e.DA_TYPE      = "TAB";
  e.className    = "DA_TAB";
  e.selected     = this.tabs.length == this.selectTab;  // $todo: (cst) set this.selectTab was 0
  e.pane         = pane;
...
//------------------------------------------------------------------------------
// $todo: (cst) new function for workaround for bouncing tabs (the selected tab is wider then the is not selected) !!!
...
domapi._private.tabset._dotabonclick = function(E){
  var p = domapi.findTarget(E, "TABSET");if(!p || !p.enabled)return;
  ...
  for(var i=0;i<t.length;i++){
    ...
    if(t[i] == e){
      p.index = i;
      if(p.onchange)p.onchange(i,t[i].value); // todo: (cst) added t[i].value to onchange function
