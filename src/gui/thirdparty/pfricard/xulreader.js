...
if (!domapi.unitLoaded("button")) throw new Error("Button unit required");
  e = domapi.Button({ parent: c, x: 0, y: 0, text: node.getAttribute("label")});
...
domapi._XulReader.prototype._listbox = function(node, c){
  ...
  nCols = node.selectSingleNode("listcols");
...
if (!domapi.unitLoaded("combobox")) throw new Error("Combobox unit required");
  e = domapi.Combobox({ parent: c, w: 100, x: 0, y: 0});

  selIndex = null;

  nPopup = node.selectSingleNode("menupopup");
  nlMenuItem = nPopup.selectNodes("menuitem");
