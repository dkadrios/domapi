...
domapi.comps.barchart.constructor = function(arg){//parent,theme,x,y,w,h){
  ...
  e.sort4       = t._sort4;
  e.vText       = t._vText;
  e.addChart    = t._addChart;
  e.addData     = t._addData;
  e.addTitle    = t._addTitle;
  e.setGrid     = t._setGrid;
  e.setLabels   = t._setLabels;
  e.setNames    = t._setNames;
  e.setColors   = t._setColors;
  e.setOrient   = t._setOrient;
  e.setXAxis    = t._setXAxis;
  e.setYAxis    = t._setYAxis;
  e.clearNames  = t._clearNames;
  e.clearTitle  = t._clearTitle;
  e.clearXAxis  = t._clearXAxis;
  e.clearYAxis  = t._clearYAxis;
  e.drawBars    = t._drawBars;
  e.drawChart   = t._drawChart;
  e.reDraw      = t._reDraw;
  e.dataSort    = t._dataSort;

  domapi._finalizeComp( e);
  return e;
};

domapi.comps.barchart._sort1=function(a,b){return (a[0] > b[0])?-1:1;};
domapi.comps.barchart._sort2=function(a,b){return (a[0] < b[0])?-1:1;};
domapi.comps.barchart._sort3=function(a,b){return (a[1] < b[1])?-1:1;};
domapi.comps.barchart._sort4=function(a,b){return (a[1] > b[1])?-1:1;};

domapi.comps.barchart._alG=function(a){    return Math.round((((((this.isVert)?(this.chart.getH()):(this.chart.getW()))-2)/(this.lines))*(a+1)*(10/11)-1));   };
domapi.comps.barchart._alM=function(a){    return Math.round((((this.isVert)?(this.chart.getH()):(this.chart.getW()))-2)*(a/this.range));                     };
domapi.comps.barchart._alH=function(){     return this.chart.getY()+this.chart.getH();                                                                        };
domapi.comps.barchart._alW=function(){     return (((this.isVert)?(this.chart.getW()):(this.chart.getH())-2)/(this.data.length))/2;                           };
domapi.comps.barchart._alN=function(a,p){  return Math.round(((a*2*this.alW())+this.alW())-(p/4));                                                            };
domapi.comps.barchart._alB=function(a){    return Math.round((this.data[i][0]*((((this.isVert)?(this.chart.getH()):(this.chart.getW()))-2)/this.range))+2);   };
domapi.comps.barchart._alA=function(t){    return Math.round(this.chart.getY()+this.chart.getH()*0.5-t.length*0.5*(this.axisSize-1));                         };

domapi.comps.barchart._vText=function(s,o){
...
domapi.comps.barchart._addChart=function(x,y,w,h,l,q){
...
domapi.comps.barchart._addData=function(set,name,del){
...
domapi.comps.barchart._addTitle=function(t,x,y){
...
domapi.comps.barchart._setGrid=function(l,q){
  if(!this.isChart)return;
  q=domapi.rInt(q,this.lType);
  this.lType=q;
  l=domapi.rInt(l,this.lines);
...
domapi.comps.barchart._setLabels=function(l,p,r){
  if(!this.isChart)return;
  l=domapi.rInt(l,this.lines);
...
domapi.comps.barchart._setNames=function(p){
...
domapi.comps.barchart._setColors=function(c,d){
  if(!this.isChart)return;
  if(c==null)
    this.isColors=false;
  else{
...
domapi.comps.barchart._setOrient=function(x){
  x=domapi.rInt(x,0);
  this.isVert=((x==0)?true:false);
  this.draw();
};

domapi.comps.barchart._setXAxis=function(t,x,y){
...
domapi.comps.barchart._setYAxis=function(t,x,y){
...
domapi.comps.barchart._clearNames=function(){                    // Used to clear the bar-names
  var t=this.childNodes[1];
  var z=t.childNodes.length;                
  this.isNames=false;
  if(z>0)
...
domapi.comps.barchart._drawChart=function(){
  this.chart.setB(1);
  var w=this.chart.style;
...
domapi.comps.barchart._draw=function(){
  var t = domapi.theme;
  var f = t.fonts;
  var s = this.style;
...
domapi.comps.barchart._dataSort=function(t){
